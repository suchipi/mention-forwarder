...
The example config spells out every field at its default, so you can delete whatever you do not need; only `command` is required. `github.allowedSources` is the one worth deleting unless you mean to pin it, since setting it stops the daily refresh of GitHub's ranges — or keep `"...default"` in the list, which adds to the published ranges instead of replacing them and leaves the refresh running.
...
| Deliveries come back `403` | The source check refused them. The log names the address it saw. If your tunnel or proxy is not loopback or on a private network, add it to `trustedProxies`; if the platform has moved to a new range, add it to `<platform>.allowedSources` — as `["...default", "<the address>"]`, so the published ranges stay in and GitHub's daily refresh keeps running. |
